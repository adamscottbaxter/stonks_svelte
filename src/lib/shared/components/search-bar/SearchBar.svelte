<script>
    ...
    function handleClick(event) {
      // fire event named 'submitted'
      value = event.target.value
      dispatch('submitted', {data: value})
    }
</script>

<div class="flex relative mx-auto w-1/4 max-w-md">
    <input class="border-2 border-primary bg-red transition h-12 px-5 pr-16 rounded-md focus:outline-none w-full text-black text-lg " type="search" name="search" placeholder="Search" value={value}  on:change={handleClick}/>
